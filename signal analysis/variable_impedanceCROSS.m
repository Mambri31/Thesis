...
% Using cross-correlation with normalization and interpolation on fixed time grid
...
function [Kt,Bt]=variable_impedanceCROSS(qh,qk,rangeK,rangeB,t,cycle_duration)
    ...
    for l=1:n_phase-1
        ...
        for k=1:nk
            for b=1:nb
                ...
                qh_interp=interp1(linspace(0,cycle_duration,length(qh{b,k})),qh{b,k},t,'makima','extrap');
                qk_interp=interp1(linspace(0,cycle_duration,length(qk{b,k})),qk{b,k},t,'makima','extrap');
        ...
        best=signalCROSS_analyzer(matrix_qh,matrix_qk,rangeK,rangeB,t_phase,cycle_duration);
    ...
    Kt=interp1(center_times,Ki,1:sample,'makima','extrap')';
    Bt=interp1(center_times,Bi,1:sample,'makima','extrap')';
